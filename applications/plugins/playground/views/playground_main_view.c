...
void playground_main_view_draw(Canvas* canvas, PlayGroundMainViewModel* model) {
    ...
    if(size > 0) {
        for(size_t item_position = 0; item_position < size; item_position++) {
            PlaygroundTextItem* current =
                PlaygroundTextItemArray_get(m->items->data, item_position);
...
bool playground_main_view_input(InputEvent* event, void* context) {
    ...
    with_view_model(
        instance->view, PlayGroundMainViewModel * model, { index = model->index; }, false);
...
void playground_main_view_set_index(PlayGroundMainView* instance, uint8_t idx) {
    furi_assert(instance);
    with_view_model(
        instance->view, PlayGroundMainViewModel * model, { model->index = idx; }, true);
}
...
uint8_t playground_main_view_get_index(PlayGroundMainView* instance) {
    ...
    uint8_t idx = 0;
    with_view_model(
        instance->view, PlayGroundMainViewModel * model, { idx = model->index; }, false);
