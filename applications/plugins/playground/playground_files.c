...
bool read_int32_playground(Stream* stream, int32_t* _data, const uint16_t data_size) {
    bool result = false;
    result = true;
    FuriString* value;
...
bool write_file_data_playground(
    Stream* src,
    Stream* file,
    bool* is_negative_start,
    size_t* current_position) {
...
bool write_file_split_playground(
    ...
    do {
        ...
        if(!stream_seek_to_key_playground(src, RAW_KEY_NAME, false)) {
        ...
        if(!write_file_noise_playground(file, is_negative_start, 0, true)) {
            FURI_LOG_E(TAG, "Add end noise failed!");
            break;
        }
